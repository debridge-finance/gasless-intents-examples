...
// Swaps USDC from Solana to Arbitrum and, during the fill itself, deposits part of
// the outcome into Aave v3 on behalf of the sender via an external call (dlnHook).
// The hook lives entirely on the EVM destination, so it is identical to the
// deposit-to-aave example — only the source side (intent + signature) is Solana.
...
const AAVE_POOL_ABI = parseAbi(["function supply(address asset, uint256 amount, address onBehalfOf, uint16 referralCode)"]);
...
async function main() {
  ...
  // The Solana keypair signs the source-chain intent; the EVM account only
  // receives funds and Aave aTokens on Arbitrum.
  ...
  const usdcSolanaToUsdcArbitrum: Trade = {
  ...
  const requestBody: BundleProposeBody = {
    ...
    trades: [usdcSolanaToUsdcArbitrum],
    postHooks: [],
  };
