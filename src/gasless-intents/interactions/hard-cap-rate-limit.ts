/**
 * Hard-cap rate-limit demo (Base USDC → Base USDT, same-chain).
 *
 * The pre-interaction targets `FillCapEnforcer`, which decodes
 * `abi.encode(address subject)` and reverts with
 * `HardCapExceeded(subject, cap, attempted)` once the per-intent fill counter
 * exceeds the subject's cap. Compare to `rate-limit-observer.ts`, which uses
 * the LoggingInteractionHook *soft* cap (no revert).
 *
 * Pre-flight: the FillCapEnforcer owner must have called
 * `setCap(account.address, N)` for some N. See
 * `contract-utils/fill-cap-enforcer/fill-cap-enforcer-set-cap.ts`.
 */
...
import { encodeSubjectPayload } from "./helpers/log-payload";
...
async function main(): Promise<Bundle> {
  ...
  const enforcer = requireHookAddress("FillCapEnforcer");
