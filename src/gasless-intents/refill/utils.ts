import { createPublicClient, formatEther, http, WalletClient } from "viem";
...
export async function waitForApprovalGas(options: WaitForApprovalGasOptions): Promise<ApprovalGasCheck | undefined> {
  for (let attempt = 1; attempt <= options.maxAttempts; attempt++) {
    const gasCheck = await getApprovalGasCheck(options.approval, options.authorityAddress);
