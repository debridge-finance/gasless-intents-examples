...
const SCENARIO = "direct-deferred";
const DEFERRED_AMOUNT = 2_000_000n;

/**
 * Recreates verified-examples/prehook direct + deferred.json. hook.type=direct,
 * placeholder.type=deferred → propose response carries a ProvidePlaceholders
 * action (no signature, only providedData supplied at submit). Saves
 * propose / submit-request / submit-response / fulfillment under
 * verified-recreated/.
 */
async function main() {
