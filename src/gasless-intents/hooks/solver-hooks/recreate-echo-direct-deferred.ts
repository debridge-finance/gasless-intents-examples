...
function findExecutionTxHash(fulfillment: unknown): `0x${string}` | null {
  // Try the trade-bearing path first (per prior eager-direct-fulfillment.json shape):
  //   execBatches[0].trades[0].srcTx.transactionHash
  // For our hook-only bundle (trades:[]) the path may differ, so walk execBatches[*]
  // looking for any object with a transactionHash / txHash field.
...
async function assertOnChainEvent(
  ...
  for (const log of receipt.logs) {
    ...
    try {
      ...
      console.error(`[${SCENARIO}] ❌ MessageEchoed mismatch:`, {
        actual: args,
        expected,
      });
      return false;
...
async function main() {
  ...
  // For hook-only bundles (no trades), the explorer's terminal status is
  // `partially_fulfilled` — there's no "trade fulfillment" to do beyond the
  // gas-comp pull. We treat both `fulfilled` and `partially_fulfilled` as
  // candidate success and gate on the on-chain MessageEchoed event for the
  // actual demo criterion.
  ...
  if (!txHash) {
    console.warn(
      `[${SCENARIO}] ⚠ Could not locate execution tx hash in fulfillment — skipping on-chain event check. Inspect echo-direct-deferred-fulfillment.json directly.`,
    );
    if (status === "fulfilled") {
